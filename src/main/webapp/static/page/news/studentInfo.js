layui.config({
	...
	//加载页面数据
	var newsData = '';
	$.get("../../json/studentInfo.json", function(data){
		var newArray = [];
		//单击首页“待审核文章”加载的信息
            	...
            	$.ajax({
					...
					success : function(data){
						...
						for(var i=0;i<newsData.length;i++){
		            		...
		            		//作者姓名
		            		if(newsStr.newsName.indexOf(selectStr) > -1){
			            		newsStr["productAuthor"] = changeStr(newsStr.productAuthor);
		            		}
		            		//指导老师
		            		if(newsStr.newsName.indexOf(selectStr) > -1){
			            		newsStr["teacher"] = changeStr(newsStr.teacher);
		            		}
		            		//作品名称
		            		...
		            		//项目数
		            		if(newsStr.number.indexOf(selectStr) > -1){
			            		newsStr["number"] = changeStr(newsStr.number);
		            		}
		            		//密码
		            		if(newsStr.password.indexOf(selectStr) > -1){
			            		newsStr["password"] = changeStr(newsStr.password);
		            		}
		            		//发布时间
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
			    	...
			    	+'<td>'+currData[i].productAuthor+'</td>'
			    	+'<td>'+currData[i].teacher+'</td>'
			    	+'<td>'+currData[i].newsStatus+'</td>'
			    	+'<td>'+currData[i].number+'</td>'
			    	+'<td>'+currData[i].password+'</td>'


//			    	if(currData[i].newsStatus == "未上传"){
