layui.config({
	...
	//加载页面数据
	var newsData = '';
	$.get("../../json/applyMasage.json", function(data){
		var newArray = [];
		//单击首页“待审核文章”加载的信息
...
//		            		}
		            		//赛事级别
		            		if(newsStr.Level.indexOf(selectStr) > -1){
			            		newsStr["Level"] = changeStr(newsStr.Level);
		            		}
		            		//赛事简介
		            		if(newsStr.newsIntro.indexOf(selectStr) > -1){
			            		newsStr["newsIntro"] = changeStr(newsStr.newsIntro);
		            		}
		            		//赛事时间
		            		...
		            		//图片
		            		if(newsStr.photo.indexOf(selectStr) > -1){
			            		newsStr["photo"] = changeStr(newsStr.photo);
		            		}
		            		//发布时间
...
//详情
	$("body").one("click",".news_collect",function(){
		$(".news_collect").click(function(){
			var index = layui.layer.open({
				title : "添加文章",
				type : 2,
				content : "applyMasageAdd.html",
				success : function(layero, index){
					setTimeout(function(){
						layui.layer.tips('点击此处返回文章列表', '.layui-layer-setwin .layui-layer-close', {
							tips: 3
...
//					+'<td>'+currData[i].photo+'</td>'
			    	...
			    	+'<td>'+currData[i].newsIntro+'</td>'
			    	+'<td>'+currData[i].time+'</td>';
					if(currData[i].newsTime == "未通过"){
			    		dataHtml += '<td style="color:#f00">'+currData[i].newsTime+'</td>';
			    	}else if(currData[i].newsTime == "已通过"){
			    		dataHtml += '<td style="color:green">'+currData[i].newsTime+'</td>';
			    	}
					else{
...
//					+  '<a class="layui-btn layui-btn-mini news_edit"><i class="iconfont icon-edit"></i> 编辑</a>'
					+  '<a class="layui-btn layui-btn-normal layui-btn-mini news_collect"><i class="layui-icon">&#xe600;</i> 查看</a>'
