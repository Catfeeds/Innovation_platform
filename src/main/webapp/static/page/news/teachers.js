layui.config({
	...
	//加载页面数据
	var newsData = '';
	$.get("../../json/teacher.json", function(data){
		var newArray = [];
		//单击首页“待审核文章”加载的信息
            	...
            	$.ajax({
					...
					success : function(data){
						...
						for(var i=0;i<newsData.length;i++){
		            		...
		            		//方向
		            		if(newsStr.direction.indexOf(selectStr) > -1){
			            		newsStr["direction"] = changeStr(newsStr.direction);
		            		}
		            		//联系电话
		            		if(newsStr.tel.indexOf(selectStr) > -1){
			            		newsStr["tel"] = changeStr(newsStr.tel);
		            		}
		            		//邮箱
		            		if(newsStr.Email.indexOf(selectStr) > -1){
			            		newsStr["Email"] = changeStr(newsStr.Email);
		            		}
		            		//图片
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
			    	...
			    	dataHtml += '<td>'+'<img  src="'+currData[i].photo+'">'+'</td>';
//			    	dataHtml +='<td>'+currData[i].photo+'</td>'
			    	dataHtml +='<td>'+currData[i].direction+'</td>'
